...
void _runStateTests(SnipThemeMode mode) {
  ...
  testWidgets('[$label] pumps cleanly with the framing section expanded', (
    tester,
  ) async {
  ...
  testWidgets(
    '[$label] every AppColors.framingGradients preview keeps its real gradient',
    (tester) async {
      await _pump(tester, mode: mode);
      await _expandFramingWithPadding(tester);
      ...
      for (final gradient in AppColors.framingGradients) {
        expect(
          rendered.any(
            (g) => g.colors.first == (gradient as LinearGradient).colors.first,
          ),
          isTrue,
          reason:
              '$label: gradient preview must render the real, unconverted stops',
        );
      }
    },
  );
  ...
  testWidgets(
    '[$label] the selected gradient ring uses ringOnGradient, not a fixed white',
    (tester) async {
      await _pump(tester, mode: mode);
      await _expandFramingWithPadding(tester);
      ...
      final border =
          (selectedTile.decoration as BoxDecoration).border as Border;
      expect(
        border.top.color,
        expectedRing,
        reason: '$label: ring must equal ringOnGradient(stops)',
      );
      expect(
        border.top.color,
        isNot(Colors.white),
  ...
  testWidgets('[$label] PNG/JPEG format tiles use the exclusive-active plate', (
    tester,
  ) async {
    await _pump(tester, mode: mode);
    ...
    expect(
      jpegLabel.style?.color,
      t.ink,
      reason: '$label: unselected format tile text must be plain ink',
    );
  });

  testWidgets('[$label] Save fires onConfirm with the entered name', (
    tester,
  ) async {
    SaveOptions? result;
    ...
    (tester) async {
      ...
      for (final icon in [
        ...
        expect(
          rendered.color,
          t.ink,
          reason: '$label: $icon must be tokenised to t.ink',
        );
        ...
        expect(
          ...
          reason:
              '$label: $icon only clears ${contrast.toStringAsFixed(2)}:1 against surfaceRaised',
        );
      }
    },
  );

  testWidgets('[$label] header chrome routes through SnipTheme', (
    tester,
  ) async {
    await _pump(tester, mode: mode);
