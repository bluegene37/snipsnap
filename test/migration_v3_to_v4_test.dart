...
void main() {
  ...
  test(
    'the v3 -> v4 migration runs and defaults existing rows to viewport',
    () async {
      ...
      for (final item in items) {
        expect(
          await _storedSpace(db, item.id),
          CoordSpace.viewport.name,
          reason:
              'the added column must default pre-v4 rows to viewport; '
  ...
  test(
    'converting one capture and re-saving it stamps only that capture',
    () async {
      final items = await DatabaseService.loadCapturesFromDb();
  ...
  test(
    ...
    () async {
      ...
      for (final id in ['legacy_a', 'legacy_b']) {
        expect(
          await _storedSpace(db, id),
          CoordSpace.viewport.name,
          reason:
              '$id was never converted, so saveHistory must not claim it '
              'is in image pixels',
        );
        expect(
          await _storedStartX(db, id),
          closeTo(100.0, 1e-6),
      ...
      for (final id in ['legacy_a', 'legacy_b']) {
        expect(
          reloaded.firstWhere((i) => i.id == id).annotationsNeedConversion,
          isTrue,
        );
      }
      expect(
  ...
  test(
    'the guard withholds annotation rows only, not capture metadata',
    () async {
  ...
  test(
    'a capture with no annotations is never flagged, so it always saves',
    () async {
      ...
      final reloaded = await DatabaseService.loadCapturesFromDb();
      expect(
        reloaded.firstWhere((i) => i.id == 'empty').annotationsNeedConversion,
        isFalse,
      );
    },
  );
