...
Future<int> _pressCmdC(WidgetTester tester, {required bool guarded}) async {
  var fired = 0;
...
void main() {
  testWidgets('an app-level chord reaches a focused text field unless guarded', (
    ...
    // selected characters.
    expect(
      await _pressCmdC(tester, guarded: false),
      1,
      reason:
          'unguarded, the app binding pre-empts the text field — this is '
  ...
  testWidgets('the focus guard stops it', (tester) async {
    expect(
      await _pressCmdC(tester, guarded: true),
      0,
