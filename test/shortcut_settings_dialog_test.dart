...
void _runStateTests(SnipThemeMode mode) {
  ...
  testWidgets('[$label] pumps cleanly and lists every shortcut action', (
    tester,
  ) async {
  ...
  testWidgets(
    '[$label] tapping Edit puts a row into the active-plate "press hotkey" state',
    (tester) async {
      await _pump(tester, mode: mode);
      ...
      expect(
        ...
        t.onActive,
        reason: '$label: pill text is the onActive knockout',
      );

      final pillContainer = tester.widget<Container>(
        find
            .ancestor(
              of: find.text('Press hotkey...'),
  ...
  testWidgets(
    '[$label] a real shortcut collision shows the danger-toned error banner, '
    'never an inline red',
    (tester) async {
      ...
      expect(
        ...
        reason: '$label: banner fill',
      );
      expect(
        decoration.border!.top.color,
        t.danger.withValues(alpha: 0.4),
        reason: '$label: banner border',
      );
  ...
  testWidgets('[$label] Reset Defaults restores the stock shortcuts', (
    tester,
  ) async {
    await _pump(tester, mode: mode);
