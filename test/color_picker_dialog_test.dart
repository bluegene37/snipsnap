...
void main() {
  testWidgets('dragging the saturation/value field repaints the indicator', (
    tester,
  ) async {
    ...
    expect(
      after.first.shouldRepaint(before.first),
      isTrue,
      reason:
          'the saturation/value field must repaint, or its indicator ring '
  ...
  testWidgets('the hue rail moves the hue and repaints', (tester) async {
    ...
    await tester.tapAt(
      Offset(body.left + body.width * 0.7, railY),
      kind: PointerDeviceKind.mouse,
    );
    await tester.pumpAndSettle();
    ...
    expect(
      HSVColor.fromColor(emitted.last).hue,
  ...
  testWidgets('the properties panel shows the active colour as hex', (
    tester,
  ) async {
