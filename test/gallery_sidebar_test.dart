...
void _runStateTests(SnipThemeMode mode) {
  ...
  testWidgets('[$label] empty state pumps cleanly and shows the hint text', (tester) async {
    await _pump(tester, mode: mode, items: const []);
    expect(find.text('No captures yet. Click "Snip" to start!'), findsOneWidget);
  });

  testWidgets('[$label] a populated tray pumps cleanly with no Image.file hang', (tester) async {
  ...
  testWidgets('[$label] the selected capture uses selectedFill, not the exclusive plate',
      (tester) async {
    ...
    expect(containers.any((d) => d.color == t.selectedFill), isTrue,
        reason: '$label: the selected card must use selectedFill');
    expect(containers.any((d) => d.color == t.surfaceRaised), isTrue,
        reason: '$label: the unselected card must use surfaceRaised, not selectedFill');
    // Never the app's exclusive-active plate — that stays reserved for the
    // tool sidebar's single active tool.
    expect(containers.any((d) => d.color == t.activeFill), isFalse,
        reason: '$label: gallery selection must never use the exclusive activeFill plate');
  });

  testWidgets('[$label] delete routes through the danger tone, never an inline red',
      (tester) async {
    final items = [_item('a')];
    ...
    expect(deleteButton.color, t.danger, reason: '$label: delete icon must use SnipTheme.danger');
  ...
  testWidgets('[$label] tapping a card fires onSelectItem', (tester) async {
    ...
    await _pump(tester, mode: mode, items: items, onSelectItem: (i) => selected = i);
  ...
  testWidgets('[$label] the extension badge uses the ink/onActive plate over the thumbnail',
      (tester) async {
    final items = [_item('a')];
    await _pump(tester, mode: mode, items: items);
    ...
    final badgeContainer = tester.widget<Container>(
      find.ancestor(of: find.text('png'), matching: find.byType(Container)).first,
    );
