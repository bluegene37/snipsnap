...
void _runStateTests(SnipThemeMode mode) {
  ...
  testWidgets('[$label] shows the recognised text and both actions', (
    tester,
  ) async {
  ...
  testWidgets('[$label] an empty result reports that nothing was found', (
    tester,
  ) async {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.text('Linux does not provide an OCR engine.'),
        findsOneWidget,
      );
  ...
  testWidgets(
    '[$label] a stale non-empty result cannot leak actions while loading',
    (tester) async {
      await _pump(
        tester,
        mode: mode,
        result: _resultWith(['stale']),
        isLoading: true,
      );
  ...
  group('[$label] chrome tokens', () {
    testWidgets('panel chrome routes through SnipTheme, not a fixed hue', (
      tester,
    ) async {
